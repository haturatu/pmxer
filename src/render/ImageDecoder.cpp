...
std::array<std::array<std::uint8_t, 4>, 4> colorTable(const std::uint8_t *block, bool allowTransparent) {
...
std::array<std::uint8_t, 8> alphaTable(const std::uint8_t *block) {
    ...
    if (alpha[0] > alpha[1]) {
        for (unsigned i = 1; i <= 6; ++i)
            alpha[i + 1] = static_cast<std::uint8_t>(((7U - i) * alpha[0] + i * alpha[1]) / 7U);
    } else {
        for (unsigned i = 1; i <= 4; ++i)
            alpha[i + 1] = static_cast<std::uint8_t>(((5U - i) * alpha[0] + i * alpha[1]) / 5U);
        alpha[6] = 0;
        alpha[7] = 255;
    }
    return alpha;
...
void writePixel(DecodedImage &image, std::uint32_t x, std::uint32_t y,
    ...
    std::copy(color.begin(), color.end(), image.rgba.begin() + static_cast<std::ptrdiff_t>(offset));
}

DecodedImage decodeDds(const std::vector<std::uint8_t> &bytes) {
    DecodedImage image;
    if (bytes.size() < 128 || read32(bytes, 0) != fourCc('D', 'D', 'S', ' ') || read32(bytes, 4) != 124 ||
        read32(bytes, 76) != 32) {
    ...
    for (std::uint32_t by = 0; by < blocksHigh; ++by) {
        for (std::uint32_t bx = 0; bx < blocksWide; ++bx, block += blockSize) {
            ...
            const auto alpha = bc3 ? alphaTable(block) : std::array<std::uint8_t, 8>{};
            ...
            for (unsigned pixel = 0; pixel < 16; ++pixel) {
                ...
                if (bc2) {
                    const auto nibble = static_cast<std::uint8_t>((block[pixel / 2U] >> (4U * (pixel & 1U))) & 15U);
...
DecodedImage decodeImageBytes(const std::vector<std::uint8_t> &bytes) {
    ...
    image.rgba.assign(pixels, pixels + static_cast<std::size_t>(width) * static_cast<std::size_t>(height) * 4U);
    stbi_image_free(pixels);
    return image;
}
...
DecodedImage decodeImage(const std::filesystem::path &path) {
    ...
    const auto fileSize = static_cast<std::uint64_t>(static_cast<std::streamoff>(end));
