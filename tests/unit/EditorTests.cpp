...
#include "../../src/editor/tools/SdefTool.hpp"
...
int main() {
  ...
  pmxer::applyViewportShadingPreset(mmdLighting,
                                    pmxer::ViewportShadingMode::mmd);
  ...
  malformedWeights.vertices[0].weights = {
      std::numeric_limits<float>::quiet_NaN(), -1.0F, 0.0F, 0.0F};
  pmxer::DocumentSession malformedWeightSession(std::move(malformedWeights));
  assert(pmxer::normalizeWeights(malformedWeightSession).success);
  const auto &normalizedVertex =
      malformedWeightSession.document.model().vertices[0];
  ...
  pmxer::DocumentSession malformedSdefSession(std::move(malformedSdef));
  assert(pmxer::hasSuspiciousSdef(
      malformedSdefSession.document.model().vertices[0]));
  const auto sdefHandle = malformedSdefSession.document.vertexHandle(0);
  const auto repairReport =
      pmxer::repairSuspiciousSdef(malformedSdefSession, {sdefHandle});
  ...
  duplicatePreview.setMorphPreview(duplicatePreviewDocument.morphHandle(1),
                                   0.5F);
  ...
  assert(std::abs(overridePreview.evaluate().vertices[0].position[0] - 0.5F) <
         1e-6F);
  pmxer::PreviewController temporaryVertexPreview(overrideModel);
  temporaryVertexPreview.setVertexPreview({previewOffset});
  assert(std::abs(temporaryVertexPreview.evaluate().vertices[0].position[0] -
                  2.0F) < 1e-6F);
  ...
  mixerSession.selection.set(
      pmxer::SelectionItem{pmxer::SelectionKind::vertex,
                           mixerSession.document.vertexHandle(0).domain,
                           mixerSession.document.vertexHandle(0).id,
                           mixerSession.document.vertexHandle(0).generation});
  pmxer::morph::setBlend(mixerSession, mixerSession.document.morphHandle(0),
                         0.5F);
  ...
  bonePreviewSession.deform.bones.push_back(
      {bonePreviewSession.document.boneHandle(0),
       {1.0F, 0.0F, 0.0F},
       {0.0F, 0.0F, 0.0F, 1.0F}});
  ...
  assert(std::abs(bonePreview.evaluate().vertices[0].position[0] - 1.0F) <
         1e-6F);
  ...
  orderedPreviewSession.deform.bones.push_back(
      {orderedBone,
       {},
       {0.0F, 0.0F, std::sin(0.25F * pi), std::cos(0.25F * pi)}});
  pmxer::PreviewController orderedPreview(orderedPreviewSession.document);
  orderedPreview.setVertexPreview(
      pmxer::vertexMorphOffsets(orderedPreviewSession));
  ...
  const auto materialInvert = [](float factor) {
    ...
    for (std::size_t vector = 0;
         vector < pmxer::morph::materialMorphVectorCount; ++vector)
  ...
  assert(std::abs(invertedMultiply.data.offsets[0].materialVectors[0][0] -
                  0.5F) < 1e-6F);
  for (const auto component :
       invertedMultiply.data.offsets[0].materialVectors[7])
    assert(component == 0.0F);
  const auto invertedFraction = pmxer::morph::invert(materialInvert(0.5F));
  assert(invertedFraction.success);
  assert(std::abs(invertedFraction.data.offsets[0].materialVectors[0][0] -
                  2.0F) < 1e-6F);
  const auto scaledMultiply = pmxer::morph::scale(materialInvert(2.0F), 0.5F);
  assert(std::abs(scaledMultiply.offsets[0].materialVectors[0][0] - 1.5F) <
         1e-6F);
  ...
  const auto subtractedZero =
      pmxer::morph::subtract(materialInvert(1.0F), materialInvert(0.0F));
  ...
  const auto split = pmxer::morph::splitSide(operationModel, operationSource,
                                             {.0F, 0.1F, false, false});
  ...
  noOpBoneSession.deform.bones.push_back(
      {noOpBoneSession.document.boneHandle(0), {}, {0.0F, 0.0F, 0.0F, 1.0F}});
  assert(
      !pmxer::morph::captureBoneMorph(noOpBoneSession, "no-op bone").success);
  ...
  auto changedBoneValue = *changedBoneSession.document.resolve(changedBone);
  changedBoneValue.flags = 0x0100U;
  assert(pmxer::editBone(changedBoneSession, changedBone, changedBoneValue)
             .success);
  const auto changedBoneCapture =
      pmxer::morph::captureBoneMorph(changedBoneSession, "changed bone");
  assert(!changedBoneCapture.success);
  ...
  pmxer::DocumentSession changedIkTargetSession(
      std::move(changedIkTargetModel));
  ...
  const auto changedIkTargetHandle =
      changedIkTargetSession.document.boneHandle(1);
  auto changedIkTargetValue =
      *changedIkTargetSession.document.resolve(changedIkTargetHandle);
  ...
  assert(pmxer::editBone(changedIkTargetSession, changedIkTargetHandle,
             ...
             .success);
  const auto changedIkTargetCapture = pmxer::morph::captureBoneMorph(
      changedIkTargetSession, "changed IK target");
  ...
  assert(pmxer::applyTransaction(
             ...
             [&](auto &transaction) {
               return static_cast<bool>(
                   transaction.addRigidBody(changedPhysicsBody));
             },
  ...
  pmxer::morph::setBlend(soloSession, soloSession.document.morphHandle(0),
                         0.5F);
  pmxer::morph::setBlend(soloSession, soloSession.document.morphHandle(1),
                         0.8F);
  ...
  assert(std::abs(
             soloSession.document.model().morphs.back().offsets.front().scalar -
             0.8F) < 1e-6F);
  ...
  pmxer::morph::setBlend(mixedBakeSession,
                         mixedBakeSession.document.morphHandle(0), 1.0F);
  pmxer::morph::setBlend(mixedBakeSession,
                         mixedBakeSession.document.morphHandle(1), 1.0F);
  ...
  assert(!pmxer::morph::bakeMixAsVertexMorph(mixedBakeSession, "mixed bake")
              .success);
  assert(pmxer::morph::bakeMixAsVertexMorph(mixedBakeSession, "mixed bake",
  ...
  assert(
      pmxer::morph::bakeAndReverseBase(reverseSession, reverseMorph).success);
  assert(reverseSession.document.model().vertices[0].position[0] == 0.0F);
  assert(reverseSession.document.model().morphs[0].offsets[0].vector3[0] ==
         -1.0F);
  ...
  pendingSaveSession.path = std::filesystem::temp_directory_path() /
                            "pmxer-pending-transform-save.pmx";
  ...
  assert(pmxer::saveDocument(pendingSaveSession, {},
                             {.allowPendingTransformEdit = true})
             .success);
  assert(pendingSaveSession.hasPendingTransformEdit());
  ...
  symmetrySession.selection.set(pmxer::SelectionItem{
      ...
      symmetrySession.document.vertexHandle(0).generation});
  std::array<float, 16> symmetryStart{};
  symmetryStart[0] = symmetryStart[5] = symmetryStart[10] = symmetryStart[15] =
      1.0F;
  pmxer::beginDeformGizmoDrag(symmetrySession, symmetryStart);
  std::array<float, 16> symmetryDelta{};
  symmetryDelta[0] = symmetryDelta[5] = symmetryDelta[10] = symmetryDelta[15] =
      1.0F;
  symmetryDelta[12] = 0.25F;
  pmxer::updateDeformGizmoDrag(symmetrySession, symmetryDelta);
  const auto leftDelta = std::find_if(
      symmetrySession.deform.vertices.begin(),
      symmetrySession.deform.vertices.end(), [&](const auto &delta) {
        return delta.vertex == symmetrySession.document.vertexHandle(0);
      });
  const auto rightDelta = std::find_if(
      symmetrySession.deform.vertices.begin(),
      symmetrySession.deform.vertices.end(), [&](const auto &delta) {
        return delta.vertex == symmetrySession.document.vertexHandle(2);
      });
  assert(leftDelta != symmetrySession.deform.vertices.end());
  assert(rightDelta != symmetrySession.deform.vertices.end());
  assert(leftDelta->offset[0] == 0.25F);
  ...
  for (std::size_t index = 0; index < 2U; ++index) {
    const auto found = std::find_if(
        ...
        mirrorGroupSession.deform.vertices.end(), [&](const auto &delta) {
          return delta.vertex ==
                 mirrorGroupSession.document.vertexHandle(index);
        });
  ...
  dragSession.selection.set(
      pmxer::SelectionItem{pmxer::SelectionKind::vertex, dragVertex.domain,
                           dragVertex.id, dragVertex.generation});
  ...
  dragCurrent[12] = 0.5F;
  pmxer::updateDeformGizmoDrag(dragSession, dragCurrent);
  assert(std::abs(dragSession.deform.vertices.front().offset[0] - 0.5F) <
         1e-6F);
  ...
  boneDragSession.selection.set(
      pmxer::SelectionItem{pmxer::SelectionKind::bone, dragBone.domain,
                           dragBone.id, dragBone.generation});
  ...
  assert(std::abs(boneDragSession.deform.bones.front().rotation[2] -
                  std::sin(5.0F * pi / 36.0F)) < 1e-5F);
  assert(std::abs(boneDragSession.deform.bones.front().rotation[3] -
                  std::cos(5.0F * pi / 36.0F)) < 1e-5F);
  ...
  vmdRotationSession.selection.set(
      pmxer::SelectionItem{pmxer::SelectionKind::bone, vmdRotationBone.domain,
                           vmdRotationBone.id, vmdRotationBone.generation});
  const auto zNinety = rotationMatrix(0.5F * pi);
  const auto xNinety = quaternionMatrix(
      {std::sin(0.25F * pi), 0.0F, 0.0F, std::cos(0.25F * pi)});
  pmxer::beginDeformGizmoDrag(vmdRotationSession, zNinety);
  pmxer::updateDeformGizmoDrag(vmdRotationSession,
                               multiplyMatrices(xNinety, zNinety));
  assert(vmdRotationSession.deform.bones.size() == 1U);
  assert(std::abs(vmdRotationSession.deform.bones.front().rotation[0]) < 1e-5F);
  assert(std::abs(vmdRotationSession.deform.bones.front().rotation[1] +
                  std::sin(0.25F * pi)) < 1e-5F);
  assert(std::abs(vmdRotationSession.deform.bones.front().rotation[2]) < 1e-5F);
  assert(std::abs(vmdRotationSession.deform.bones.front().rotation[3] -
                  std::cos(0.25F * pi)) < 1e-5F);
  ...
  const auto xThirty =
      mmd::Float4{std::sin(pi / 12.0F), 0.0F, 0.0F, std::cos(pi / 12.0F)};
  ...
  nonCommutativeSession.deform.bones.push_back(
      {nonCommutativeBone, {}, xThirty});
  nonCommutativeSession.deform.dirty = true;
  const auto yTwenty =
      mmd::Float4{0.0F, std::sin(pi / 18.0F), 0.0F, std::cos(pi / 18.0F)};
  const auto expectedNonCommutative =
      mmd::Float4{yTwenty[3] * xThirty[0] + yTwenty[1] * xThirty[2],
  ...
  for (std::size_t component = 0; component < 4U; ++component)
    assert(std::abs(
               nonCommutativeSession.deform.bones.front().rotation[component] -
  ...
  childBoneSession.selection.set(
      pmxer::SelectionItem{pmxer::SelectionKind::bone, childHandle.domain,
                           childHandle.id, childHandle.generation});
  ...
  assert(std::abs(childBoneSession.deform.bones.front().rotation[1] +
                  std::sin(0.25F * pi)) < 1e-5F);
  assert(std::abs(childBoneSession.deform.bones.front().rotation[2]) < 1e-5F);
  assert(std::abs(childBoneSession.deform.bones.front().rotation[3] -
                  std::cos(0.25F * pi)) < 1e-5F);
  ...
  const mmd::Float3 compoundEuler{20.0F * pi / 180.0F, 30.0F * pi / 180.0F,
                                  40.0F * pi / 180.0F};
  const auto legacyCompound =
      legacyMultiply(legacyMultiply(legacyAxisMatrix(0U, compoundEuler[0]),
                                    legacyAxisMatrix(1U, compoundEuler[1])),
                     legacyAxisMatrix(2U, compoundEuler[2]));
  const auto quaternionCompound =
      pmxer::composeRotationMatrix(pmxer::quaternionFromEuler(compoundEuler));
  for (std::size_t component = 0; component < 16U; ++component)
    assert(std::abs(quaternionCompound[component] - legacyCompound[component]) <
           1e-5F);
  ...
  multiBoneSession.selection.set(
      std::vector<pmxer::SelectionItem>{multiBoneItem(0), multiBoneItem(1)});
  const auto multiBoneCapabilities =
      pmxer::transformCapabilities(multiBoneSession);
  ...
  appendBoneSession.selection.set(
      pmxer::SelectionItem{pmxer::SelectionKind::bone, appendBone.domain,
                           appendBone.id, appendBone.generation});
  ...
  ikBoneSession.selection.set(pmxer::SelectionItem{pmxer::SelectionKind::bone,
                                                   ikTarget.domain, ikTarget.id,
                                                   ikTarget.generation});
  ...
  physicsBoneSession.selection.set(
      pmxer::SelectionItem{pmxer::SelectionKind::bone, physicsBone.domain,
                           physicsBone.id, physicsBone.generation});
  ...
  retainedSession.deform.vertices.push_back(
      {retainedVertex, {0.5F, 0.0F, 0.0F}});
  ...
  const pmxer::CameraState perspective{
      {0.0F, 0.0F, 0.0F}, 0.0F, 0.0F, 10.0F, false};
  const auto perspectiveCenter = pmxer::projectWorldToScreen(
      perspective, {0.0F, 0.0F, 0.0F}, 0.0F, 0.0F, 800.0F, 600.0F);
  ...
  assert(!pmxer::projectWorldToScreen(perspective, {0.0F, 0.0F, 20.0F}, 0.0F,
                                      0.0F, 800.0F, 600.0F)
              .inFront);
  auto orthographic = perspective;
  orthographic.orthographic = true;
  const auto orthographicCenter = pmxer::projectWorldToScreen(
      orthographic, {0.0F, 0.0F, 0.0F}, 0.0F, 0.0F, 800.0F, 600.0F);
  ...
  recovered.selection.set({pmxer::SelectionKind::material,
  ...
  assert(
      pmxer::editVertex(recovered, recoveredHandle, recoveredVertex).success);
  assert(recovered.undo());
  assert(recovered.selection.contains(
      {pmxer::SelectionKind::material, recoveredMaterial.domain,
  ...
  const auto modelPolicy =
      pmxer::workspacePolicy(pmxer::EditorWorkspace::model);
  ...
  const auto morphPolicy =
      pmxer::workspacePolicy(pmxer::EditorWorkspace::morph);
  ...
  pmxer::applyWorkspacePolicy(
      session, pmxer::workspacePolicy(pmxer::EditorWorkspace::rig));
  ...
  pmxer::applyWorkspacePolicy(
      session, pmxer::workspacePolicy(pmxer::EditorWorkspace::physics));
  ...
  pmxer::selectPrimary(synchronized, synchronizedWorkspace,
                       {pmxer::SelectionKind::bone, synchronizedBone.domain,
                        synchronizedBone.id, synchronizedBone.generation},
                       pmxer::SelectionOrigin::viewport);
  assert(synchronizedWorkspace == pmxer::EditorWorkspace::rig);
  assert(synchronized.selection.items().front().kind ==
         pmxer::SelectionKind::bone);
  assert(pmxer::actionAvailability(pmxer::EditorAction::viewportRotate,
                                   synchronized)
             .support == pmxer::SupportLevel::unsupported);

  const auto boneItem =
      pmxer::SelectionItem{pmxer::SelectionKind::bone, boneForPolicy.domain,
                           boneForPolicy.id, boneForPolicy.generation};
  ...
  const auto weightedVertexItem =
      pmxer::SelectionItem{pmxer::SelectionKind::vertex, weightedVertex.domain,
                           weightedVertex.id, weightedVertex.generation};
  for (int index = 0; index < 4; ++index) {
    const auto bone =
        weightedSession.document.boneHandle(static_cast<std::size_t>(index));
  ...
  physicsSession.selection.set(
      {pmxer::SelectionKind::rigidBody, body.domain, body.id, body.generation});
  const auto bodyRelations = pmxer::physicsSelectionRelations(physicsSession);
  ...
  const auto jointRelations = pmxer::physicsSelectionRelations(physicsSession);
  ...
  assert(pmxer::selectMorphOffsetTarget(morphTargetSession, targetVertexItem));
  ...
  assert(pmxer::selectAllForMode(morphTargetSession, morphWorkspace,
  ...
  const auto layoutPath = std::filesystem::temp_directory_path() /
                          "pmxer-workspace-layout-test.ini";
  ...
  assert(
      !session.document.referencesTo(session.document.boneHandle(0)).empty());
  pmxer::DocumentSession otherSession(sampleModel());
  assert(session.document.resolve(otherSession.document.vertexHandle(0)) ==
         nullptr);
  ...
  assert(
      pmxer::editVertex(historySession, historyHandle, historyVertex).success);
  assert(pmxer::applyTransaction(
             historySession,
  ...
  assert(historySession.document.resolve(historyMorph)->name ==
         "history_morph");
  ...
  assert(invalidValidation.issues.front().location.kind ==
         mmd::ReferenceObjectKind::vertex);
  assert(invalidValidation.issues.front().location.id ==
         invalidDocument.vertexHandle(2).id);
  ...
  assert(morphHandle);
  assert(
      structure.addVertexMorphOffset(morphHandle, handle, {0.1F, 0.0F, 0.0F}));
  assert(structure.setVertexMorphOffset(morphHandle, 0, handle,
                                        {0.2F, 0.0F, 0.0F}));
  ...
  mmd::PmxJoint joint;
  joint.name = "joint";
  const auto jointHandle =
      structure.addJoint({joint, bodyHandleA, bodyHandleB});
  assert(jointHandle);
  ...
  const pmxer::SelectionItem item{pmxer::SelectionKind::vertex, handle.domain,
                                  handle.id, handle.generation};
  const auto id = picking.assign(item);
  assert(picking.resolve(id).value() == item);
  const auto restored =
      pmxer::selectionHandle<mmd::VertexTag>(session.document, item);
  ...
  const pmxer::SelectionItem secondItem{pmxer::SelectionKind::vertex,
                                        handle.domain, handle.id + 1U,
                                        handle.generation};
  ...
  const auto path =
      std::filesystem::temp_directory_path() / "pmxer-editor-test.pmx";
  session.path = path;
  ...
  const auto untitledDestination =
      path.parent_path() / "pmxer-untitled-save-test.pmx";
  ...
  assert(mmd::pmx::semanticEqual(session.document.model(), reloaded,
